...
class CadastrarTurma extends StatefulWidget {
  ...
  @override
  State<CadastrarTurma> createState() => _CadastrarTurmaState();
}
...
class _CadastrarTurmaState extends State<CadastrarTurma> {
  ...
  void _populateFields(Map<String, dynamic> turma) {
    _descricaoController.text = turma['descricao'];
    setState(() {});
  }

  void _loadAluno(int id) async {
    final turma = await dbHelper.getTurmaById(id);
    if (turma != null) {
      _populateFields(turma);
    }
  }
  ...
  void _saveItem() async {
    try {
      if (_formKey.currentState!.validate()) {
        ...
        if (widget.alunoId != null) {
          await dbHelper.updateTurma(widget.alunoId!, faixa);
        } else {
          await dbHelper.insertTurma(faixa);
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          widget.alunoId != null ? 'Atualizar Turma' : 'Cadastrar Turma',
          style: GoogleFonts.poppins(
            fontSize: 20,
